...
trait Diff extends LanternAPI {
    // The return type of the continuation context is Unit (ignore)
    type diff = cps[Unit]   
    // Unit @diff is a syntactic sugar for ControlContext[Unit,Unit,Unit])
    // reset creates a context for the continuation, similar to yield; transformed with calls to shift
